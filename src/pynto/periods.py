...
def _offset_b(date: datetime.date, b: int) -> datetime.date:
    ...
    return prevMonday + datetime.timedelta(
        days=b + weekendsInDistance * 2 + daysSinceMonday
    )
...
class Period:
    ...
    def __len__(self) -> int:
        return (
            self.periodicity._offset(self.periodicity.epoque, self.ordinal)
            - self.periodicity._offset(self.periodicity.epoque, self.ordinal - 1)
        ).days

    def __getitem__(self, index: int) -> datetime.date:
        if index >= 0:
            first = self.periodicity._offset(
                self.periodicity.epoque, self.ordinal - 1
...
class Range:
    ...
    def __getitem__(self, index: int | slice) -> Period | Range:
        if isinstance(index, int):
            ...
            if isinstance(index.start, int) and index.start >= 0:
                start += index.start
            elif isinstance(index.start, int) and index.start < 0:
                start = stop + index.start
            if isinstance(index.stop, int) and index.stop >= 0:
                stop = start + index.stop
            elif isinstance(index.stop, int) and index.stop < 0:
                stop += index.stop
            return Range(start, stop, self.periodicity)
        else:
            raise TypeError('Period indices must be integers or slices')

    def __len__(self) -> int:
        assert self.stop >= self.start, (
            f'Negative range length {self.start}-{self.stop}'
        )
    ...
    def to_index(self) -> pd.DatetimeIndex:
        return pd.date_range(
            self[0][-1], self[-1][-1], freq=self.periodicity.offset_code
        )
