...
class SkipList:
    ...
    def insert(self, element, score):
        ...
        for i in range(self.level, -1, -1):
            while x.forward[i] and (
                ...
                or (x.forward[i].score == score and x.forward[i].element < element)
            ):
                x = x.forward[i]
            update[i] = x
    ...
    def delete(self, element):
        ...
        for i in range(self.level, -1, -1):
            while x.forward[i] and (
                x.forward[i].score < self.dict[element].score
                or (
                    x.forward[i].score == self.dict[element].score
                    and x.forward[i].element < element
                )
            ):
                x = x.forward[i]
            update[i] = x
    ...
    def rank(self, element, reverse=False):
        ...
        for i in range(self.level, -1, -1):
            while x.forward[i] and (
                x.forward[i].score < target_score
                or (
                    x.forward[i].score == target_score
                    and x.forward[i].element < target_element
                )
            ):
                rank += 1
                x = x.forward[i]
    ...
    def range_by_lex(self, min_lex, max_lex, start=0, count=None, reverse=False):
        ...
        min_value, max_value, inclusive_min, inclusive_max = self._parse_lex_range(
            min_lex, max_lex
        )
        ...
        for i in range(self.level, -1, -1):
            while x.forward[i] and (
                x.forward[i].score < lex_score
                or (
                    x.forward[i].score == lex_score
                    and (
                        (inclusive_min and x.forward[i].element < min_value)
                        or (not inclusive_min and x.forward[i].element <= min_value)
                    )
                )
            ):
                x = x.forward[i]
        ...
        while (
            x
            and skipped < start
            and x.score == lex_score
            and (
                (inclusive_max and x.element <= max_value)
                or (not inclusive_max and x.element < max_value)
            )
        ):
            skipped += 1
            x = x.forward[0]
    ...
    def _parse_lex_range(self, min_lex, max_lex):
        ...
        if max_lex == '+':
            max_value = (
                '\uffff'  # A high Unicode value that will be greater than any string
            )
            inclusive_max = True
        ...
        for word, description, callable_obj in sorted(entries):
            ...
            try:
                ...
                if slice_obj == slice(None):
                    ...
                    slice_str = (
                        f'[:{slice_obj.stop}:{slice_obj.step}]'
                        if slice_obj.step
                        else f'[:{slice_obj.stop}]'
                    )
                elif slice_obj.stop is None:
                    slice_str = (
                        f'[{slice_obj.start}::{slice_obj.step}]'
                        if slice_obj.step
                        else f'[{slice_obj.start}:]'
                    )
                else:
                    slice_str = (
                        f'[{slice_obj.start}:{slice_obj.stop}:{slice_obj.step}]'
                        if slice_obj.step
                        else f'[{slice_obj.start}:{slice_obj.stop}]'
                    )
