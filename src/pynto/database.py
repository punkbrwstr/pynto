...
METADATA_FORMAT = '<256s128s128sLllccdd16s'
...
class Db:
    ...
    def get_metadata(self, key: str) -> list[Metadata]:
        ...
        mds = [
            Metadata.unpack(p)
            for p in self.connection.zrangebylex(INDEX, f'[{k}', f'[{k}\xff')
        ]
        if not mds:
    ...
    def __getitem__(self, args: str | tuple[str, slice]) -> pd.Frame:
        ...
        mds = [
            Metadata.unpack(d)
            for d in self.connection.zrangebylex(
                INDEX, f'[{safe_key}', f'[{safe_key}\xff'
            )
        ]
        if not mds:
    ...
    def _req(
        self, saved: Metadata, start: int, stop: int, p: redis.client.Pipeline
    ) -> int:
