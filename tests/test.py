...
class TestRowIndexing(unittest.TestCase):
    def test_int_index(self):
        ...
        with self.subTest('Date range'):
            self.assertEqual(pt.r5.rows['2023-04-01':'2023-04-05'].shape[0], 2)
        with self.subTest('Date range pre epoque'):
            self.assertEqual(pt.r5.rows['1969-12-15':'1970-01-08'].shape[0], 18)
...
class TestOtherColumnIndexingWords(unittest.TestCase):
    def test_drop(self):
        a = (pt.r5 + pt.hset('a,d,c,d,e') + pt.drop.cols['d']).rows[0]
        self.assertTrue(np.array_equal(a, np.array([[0.0, 2.0, 4.0]])))

    def test_keep(self):
        a = (pt.r5 + pt.hset('a,d,c,d,e') + pt.keep.cols['d']).rows[0]
        self.assertTrue(
            np.array_equal(
                a,
                np.array(
    ...
    def test_dup(self):
        a = (pt.r5 + pt.hset('a,d,c,d,e') + pt.dup.cols['d']).rows[0]
        self.assertTrue(
...
class TestCombinators(unittest.TestCase):
    ...
    def test_tilde_quote(self):
        result = (pt.c(5) + pt.c(6) + ~pt.add + pt.call).rows['2019-01-01'].iloc[0, -1]
        self.assertEqual(result, 11)

    def test_nested_quotes(self):
        result = (pt.r9 + ~(~(pt.c(100) + pt.sub) + pt.call) + pt.map(every=3)).rows[-1]
        self.assertTrue(
    ...
    def test_map(self):
        result = (pt.c(5) + pt.c(6) + ~(pt.c(1) + pt.add) + pt.map).rows['2019-01-01']
    ...
    def test_ifexists(self):
        expr = (
            pt.r10
            + pt.hset('c,c,a,b,a,a,b,a,a,b')
            + ~(~(pt.c(100) + pt.mul) + pt.ifexists(3) + pt.add.cols[:])
            + pt.hmap
            + pt.hsort
        )
    ...
    def test_if(self):
        expr = (
            ...
            + ~(
                ~(pt.c(100) + pt.mul)
                + pt.ifheaders(lambda length: len(length) >= 3)
                + pt.add.cols[:]
            )
            + pt.hmap
            + pt.hsort
    ...
    def test_ifelse(self):
        expr = (
            ...
            + ~(
                ~(pt.c(100) + pt.div)
                + ~(pt.c(100) + pt.mul)
                + pt.ifheaderselse(lambda length: len(length) >= 3)
                + pt.add.cols[:]
            )
            + pt.hmap
            + pt.hsort
    ...
    def test_cleave(self):
        result = (pt.c(4) + ~pt.neg + ~pt.sqrt + pt.cleave).rows[0]
        self.assertEqual(result.iloc[0, -1], 2)
        self.assertEqual(result.iloc[0, -2], -4)

    def test_partial(self):
        result = (pt.r5 + ~pt.mul + pt.partial + pt.map).last.values
        self.assertTrue(np.array_equal(result, [0.0, 4.0, 8.0, 12.0]))

    def test_compose(self):
        result = (
            pt.r5 + ~(pt.c(1) + pt.add) + ~pt.inv + pt.compose + pt.map
        ).last.values
        self.assertTrue(np.array_equal(result, [1, 0.5, 1 / 3, 1 / 4, 1 / 5]))

    def test_repeat(self):
        # Applies ~(c1 + add) three times: 0 + 1 + 1 + 1 = 3
        result = (pt.c(0) + ~(pt.c(1) + pt.add) + pt.repeat(3)).rows[0]
        self.assertEqual(result.iloc[0, -1], 3.0)


class TestHeaders(unittest.TestCase):
    def test_hset(self):
        self.assertEqual((pt.r4 + pt.hset('q', 'w', 'e', 'r')).columns[1], 'w')
        self.assertEqual((pt.r6 + pt.hset('q', 'w', 'e', 'r')).columns[1], 'c1')

    def test_halpha(self):
        self.assertEqual((pt.r10 + pt.halpha).columns[1], 'b')

    def test_hformat(self):
        self.assertEqual(
            (pt.r5 + pt.halpha + ~pt.hformat('{0}x') + pt.map).columns[1], 'bx'
        )

    def test_happly(self):
        self.assertEqual(
            (pt.r5 + pt.halpha + ~pt.happly(lambda h: h.upper()) + pt.map).columns[1],
            'B',
        )
...
class TestDataCleanup(unittest.TestCase):
    def test_join(self):
        result = (
            pt.c(1) + pt.nan + pt.join('1970-01-10') + pt.c(3) + pt.join('1970-01-15')
        ).values[6:11]
        self.assertTrue(
            np.array_equal(
    ...
    def test_ffill_lookback(self):
        ...
        result = (pt.from_pandas(df) + pt.ffill(leave_end=False)).values[
            '1970-01-10':'1970-01-16'
        ]
    ...
    def test_zero_first(self):
        ...
        result = (pt.c(5) + pt.zero_first).values[:2]
        self.assertEqual(result[0, 0], 0.0)
        self.assertEqual(result[1, 0], 5.0)

    def test_zero_to_na(self):
        # zero_to_na replaces zeros with NaN
        result = (pt.c(0) + pt.zero_to_na).values[0]
        self.assertTrue(np.isnan(result[0, 0]))


class TestUnaryOps(unittest.TestCase):
    def test_lnot(self):
        self.assertTrue(bool((pt.c(0) + pt.lnot).values[0][0, 0]))  # not 0 = True
        self.assertFalse(bool((pt.c(1) + pt.lnot).values[0][0, 0]))  # not 1 = False

    def test_sign(self):
        result = (pt.c(5) + pt.c(-3) + pt.c(0) + pt.sign.cols[:]).values[0]
        self.assertTrue(np.array_equal(result, [[1.0, -1.0, 0.0]]))

    def test_expm1(self):
        # exp(0) - 1 = 0
        result = (pt.c(0) + pt.expm1).values[0]
        self.assertAlmostEqual(result[0, 0], 0.0)

    def test_log1p(self):
        # ln(1 + 0) = 0
        result = (pt.c(0) + pt.log1p).values[0]
        self.assertAlmostEqual(result[0, 0], 0.0)

    def test_inc(self):
        result = (pt.c(5) + pt.inc).values[0]
        self.assertEqual(result[0, 0], 6.0)

    def test_dec(self):
        result = (pt.c(5) + pt.dec).values[0]
...
class TestReverseCumulative(unittest.TestCase):
    def test_rcadd(self):
        ...
        result = (pt.c(1) + pt.rcadd).values[-5:]
...
class TestRollingSpecial(unittest.TestCase):
    ...
    def test_ewm_mean(self):
        ...
        result = (pt.from_pandas(self._df([0])) + pt.ewm_mean(self.WINDOW)).values[last]
...
class TestConstantLiterals(unittest.TestCase):
    def test_int_radd(self):
        # int + Word creates a Constant and chains
        result = (5 + pt.dup + pt.mul).last
        self.assertEqual(result.values[0], 25.0)

    def test_float_add(self):
        # Word + float creates a Constant and chains
        result = (pt.c(3) + 2.5 + pt.add).last
        self.assertEqual(result.values[0], 5.5)

    def test_int_add(self):
        # Word + int creates a Constant and chains
        result = (pt.c(3) + 10 + pt.add).last
        self.assertEqual(result.values[0], 13.0)

    def test_negative_radd(self):
        result = (-5 + pt.neg).last
        self.assertEqual(result.values[0], 5.0)


class TestQuotationSyntax(unittest.TestCase):
    def test_simple_tilde(self):
        # ~word creates a quotation
        result = (pt.c(5) + pt.c(6) + ~pt.add + pt.call).rows['2019-01-01'].iloc[0, -1]
        self.assertEqual(result, 11)

    def test_expression_tilde(self):
        # ~(expr) quotes the expression
        result = (pt.c(5) + pt.c(6) + ~(pt.c(1) + pt.add) + pt.map).rows['2019-01-01']
        self.assertEqual(result.iloc[0, -1], 7)
        self.assertEqual(result.iloc[0, -2], 6)

    def test_nested_tilde(self):
        # Nested quotations
        result = (pt.r9 + ~(~(pt.c(100) + pt.sub) + pt.call) + pt.map(every=3)).rows[-1]
        self.assertTrue(
            np.array_equal(result.values[-1], [0, 1.0, -98, 3, 4, -95, 6, 7, -92])
    ...
    def test_double_invert(self):
        # ~~ should unwrap a quotation back to a standard word
        expr = pt.dup + pt.mul
        unquoted = ~~expr
        result = (pt.c(5) + unquoted).last
        self.assertEqual(result.values[0], 25.0)
...
class FrameTest(DbTest):
    def setUp(self):
        pt.db['test'] = (pt.r10 + pt.halpha).rows[:5]
    ...
    def test_overwrite(self):
        pt.db['test'] = (pt.r10 + pt.rev + pt.halpha).rows[:5]
        f = pt.db['test']
        self.assertEqual(f.values[0, 0], 9.0)

    def test_append_rows(self):
        pt.db['test'] = (pt.r10 + pt.halpha).rows[5:10]
        f = pt.db['test']
        self.assertEqual(f.shape[1], 10)
        self.assertEqual(f.shape[0], 10)

    def test_append_cols(self):
        pt.db['test'] = (pt.r2 + pt.hset('k,l')).rows[5:10]
        f = pt.db['test']
...
class IntTest(DbTest):
    def setUp(self):
        pt.db['test'] = (pt.r10 + pt.halpha).rows[:5].astype(np.int64)
...
class BoolTest(DbTest):
    def setUp(self):
        pt.db['test'] = (pt.r10 + pt.halpha).rows[:5].astype(np.bool)
...
class SeriesTest(DbTest):
    def setUp(self):
        pt.db['test'] = (pt.r10 + pt.halpha).rows[:5].iloc[:, 0]
...
class TestResample(DbTest):
    ...
    def test_per_last(self):
        ...
        result = (pt.load(self.KEY) + pt.set_periodicity('M')).rows[0:1:'M']
    ...
    def test_per_lastnofill(self):
        ...
        result = (
            pt.load(self.KEY) + pt.resample_lastnofill + pt.set_periodicity('M')
        ).rows[0:1:'M']
    ...
    def test_per_first(self):
        ...
        result = (pt.load(self.KEY) + pt.resample_first + pt.set_periodicity('M')).rows[
            1:2:'M'
        ]
        self.assertEqual(result.iloc[0, 0], 1.0)
    ...
    def test_per_min(self):
        # Feb min of 1..20 = 1.
        result = (pt.load(self.KEY) + pt.resample_min + pt.set_periodicity('M')).rows[
            1:2:'M'
        ]
        self.assertEqual(result.iloc[0, 0], 1.0)
    ...
    def test_per_max(self):
        ...
        result = (pt.load(self.KEY) + pt.resample_max + pt.set_periodicity('M')).rows[
            0:2:'M'
        ]
...
class TestSaved(DbTest):
    ...
    def setUp(self):
        pt.db[self.KEY] = (pt.r5 + pt.halpha).rows[:5]
